...
|----------|------|-------------|
| `onValueChanged` | `ScriptEvent` | Fires when the input field's text changes. Arguments: `string value`. |
| `onEndEdit` | `ScriptEvent` | Fires when the input field is done being edited. Arguments: `string value`. |
| `onPointerEnter` | `ScriptEvent` | Fires when the pointer enters the input field area. |
| `onPointerExit` | `ScriptEvent` | Fires when the pointer exits the input field area. |
...
| `onPointerEnter` | `ScriptEvent` | Triggered when the pointer enters the input field area. |
| `onPointerExit` | `ScriptEvent` | Triggered when the pointer exits the input field area. |
