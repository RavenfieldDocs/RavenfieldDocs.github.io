...
|----------|------|-------------|
| `onValueChanged` | `ScriptEvent` | Fires when the toggle state changes. Arguments: `bool value`. |
...
|-------|-----------|-------------|
| `onValueChanged` | `(value)` | Triggered when the toggle is switched on or off. The `value` parameter is `true` when the toggle is on, `false` when off. |
